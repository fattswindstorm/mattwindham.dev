# ---------------------------------------------------------------
# Persistent stack: just the GitHub Actions lifecycle role, plus the
# budget/cost-allocation tag. Deliberately separate from terraform/eks-demo/
# (the ephemeral VPC/cluster/node-group stack that spin-up-eks-demo.yml and
# teardown-eks-demo.yml apply/destroy every cycle) - this role is what those
# two workflows authenticate WITH, so if it lived in the same state as the
# resources they destroy every night, the very first teardown would delete
# the role needed to authenticate the next spin-up, permanently breaking the
# lifecycle after one cycle. Applied manually via terraform-deployer, same
# as terraform/github-oidc and terraform/billing-alert - not wired to any
# CI workflow, since this is exactly the kind of account-level IAM change
# that shouldn't auto-apply.
# ---------------------------------------------------------------
...
data "aws_iam_policy_document" "eks_demo_lifecycle_permissions" {
  ...
  statement {
    ...
    actions = [
      ...
      # the ALB controller's IRSA role (see terraform/eks-demo/cluster.tf),
      # but the workflow itself needs read access here for the post-install
      # wait/poll step.
